...
public class FaultList {
  ...
  /*  private static ErrorCodes code;
    ...
    public List<String> processLines (List<String> lines) {
        ...
        for (String line : lines) {
            String[] words = line.split(",");
            results.add(lineValidator(words));
        }
        return results;
    }

    private static String lineValidator(String[] words ){
            int ordinal = 0;
            try {
                ordinal = Integer.parseInt(words[0]);
            } catch (NumberFormatException e){
                return code.COMMENT.getCode() + " ";
            }
            if (words.length != 3){
                return ordinal + ", " + code.WORDCOUNT_ERROR.getCode();
            }
            try {
                Double.parseDouble(words[1]);
            } catch (NumberFormatException d){
                return ordinal + ", " + code.VALUE_ERROR.getCode();
            }
            try {
                ...
                return ordinal + ", " + code.DATE_ERROR.getCode();
            }
            return "";
    }
